...
function format_share_percent(?float $value): string
{
    ...
    // ไม่ใช่ศูนย์จริง แต่ปัด 2 ตำแหน่งแล้วกลายเป็น 0.00
    if ($value !== 0.0 && abs($value) < 0.005) {
        return $value > 0 ? '<0.01%' : '>-0.01%';
    }

    /* ⚠️⚠️ **สองตำแหน่ง ต่างจาก % อื่นโดยตั้งใจ** — คอลัมน์นี้เป็นคอลัมน์เดียวที่
       "ผลรวมมีความหมาย" · ค่าที่คำนวณไว้รวมกันได้ 100.00 พอดี (`distribute_profit_share()`
       แจกเศษให้) แต่ถ้าแสดงตำแหน่งเดียว 3 ร้านเท่ากันจะกลายเป็น **33.3 × 3 = 99.9%**
       บนหน้าจอ — คนอ่านบวกตามที่เห็นแล้วไม่ครบ
       · ไฟล์ Excel ใช้ `SHARE_FORMAT` (2 ตำแหน่ง) อยู่แล้ว จอกับไฟล์จึงต้องตรงกัน */
...
/**
 * ⭐⭐ วันตัดที่ **ทั้งสองเดือนใช้ร่วมกันได้จริง** — สำหรับ "เดือนนี้เทียบเดือนก่อน"
 *
 * `resolve_comparison_cutoff_day()` คืนวันที่ของวันนี้เฉย ๆ ซึ่งใช้ได้กับเดือนปัจจุบัน
 * แต่ **เดือนก่อนอาจไม่มีวันนั้น** · `comparison_range_end()` หดปลายช่วงของเดือนก่อน
 * ให้พอดีอยู่แล้ว ผลคือสองฝั่งถูกวัดคนละจำนวนวันโดยที่ป้ายยังประกาศวันเดิม
 *
 * ⚠️⚠️ **วัดจริง** (ร้านที่ทำกำไรวันละ ฿1,000 เท่ากันเป๊ะทุกวัน 3 ปี กรอกครบทุกวัน):
 *   28 มี.ค. → 0.0%   (28 วัน เทียบ 28 วัน)
 *   29 มี.ค. → +3.6%  (29 วัน เทียบ 28 วัน) ← ป้ายบอกว่า "ถึงวันที่ 29" ซึ่ง ก.พ. ไม่มี
 *   30 มี.ค. → +7.1%
 *   31 มี.ค. → +10.7% ← ป้ายบอกว่า "ถึงวันที่ 31" ซึ่ง ก.พ. ไม่มี
 *    1 เม.ย. → 0.0%   (กลับมาเอง)
 * สามวันสุดท้ายของเดือนที่ยาวกว่าเดือนก่อน ป้ายบอกว่ากำไรโตขึ้น ทั้งที่ธุรกิจไม่เปลี่ยนเลย
 * — และวันสิ้นเดือนคือวันที่คนเปิดแดชบอร์ดมากที่สุด
 *
 * ⚠️ เจตนาของฟีเจอร์นี้เขียนไว้ในโค้ดเองว่า "เทียบกับเดือนก่อน**ถึงวันเดียวกัน**"
 * การหดวันตัดให้พอดีเดือนที่สั้นกว่าจึงเป็นการทำให้โค้ดตรงกับเจตนาที่ประกาศไว้
 * ไม่ใช่การเปลี่ยนนิยามของเมตริก
 *
 * ⚠️⚠️ **ห้ามเอาไปใช้แทน `resolve_comparison_cutoff_day()` ทุกที่** — ตัวนั้นยังถูกต้อง
 * สำหรับงานที่ไม่ได้เทียบสองเดือน (ตัดยอดเดือนนี้ที่วันนี้ · ปลายกราฟ · เดือนของเป้าหมาย)
 * ตัวนี้ใช้เฉพาะจุดที่มี "เดือนก่อน" มาเกี่ยวข้องเท่านั้น
 *
 * @param string|null $today รูปแบบ Y-m-d — seam สำหรับเทสต์
 * @return int|null null = เดือนที่จบแล้ว (เทียบทั้งเดือนได้เลย)
 */
function resolve_month_over_month_cutoff_day(string $selectedMonth, ?string $today = null): ?int
{
    ...
    $previousMonthDays = (int)$monthStart->modify('-1 month')->format('t');

    return max(1, min($cutoffDay, $previousMonthDays));
}
 ...
 */
/**
 * ⭐⭐ "% เปลี่ยนแปลงเทียบช่วงก่อน" — จุดเดียวของสูตรนี้ ห้ามเขียนซ้ำที่อื่น
 *
 * ⚠️ เคยเขียนซ้ำ 4 ที่ (`AnnualService` · `DashboardService` · `OverviewAnnualService`
 * · และแบบ inline ใน `OverviewService::attachProfitMomentum`) ทุกตัวเหมือนกันเป๊ะ
 * ต่างแค่ตัวหนึ่งรับ null ได้ — สูตรที่ก๊อปไว้ 4 ที่คือจุดที่จะเพี้ยนเมื่อมีคนแก้ที่เดียว
 *
 * ⚠️ **หารด้วย `abs($previous)`** เพื่อให้เครื่องหมายสื่อทิศทางจริง:
 * ฐานติดลบแล้วขาดทุนน้อยลง = ดีขึ้น = ค่าบวก (ถ้าหารด้วยค่าติดลบตรง ๆ จะกลับหัว)
 *
 * ⚠️ **ฐาน ≈ 0 คืน null ไม่ใช่ 0 หรือ 100** — เพิ่มจาก ฿0 เป็น ฿5,000 คิดเป็นกี่ % ไม่ได้
 * ผู้เรียกต้องแยกเองว่า null แปลว่า "ไม่มีข้อมูล" หรือ "ฐานเป็นศูนย์" (คนละความหมาย)
 */
function change_percent(?float $current, ?float $previous): ?float
{
    /* ⚠️⚠️⚠️ **ฐานเป็นศูนย์เป๊ะเท่านั้นที่เทียบไม่ได้ — ห้ามใช้เกณฑ์ที่ผูกกับหน่วยเงิน**
       · เดิมเขียน `abs($previous) < 0.00001` ซึ่งเป็นเกณฑ์ที่คิดมาจาก "จำนวนเงินที่เล็ก
         จนไม่มีความหมาย" · แต่ฟังก์ชันนี้ถูกเรียกด้วย **ROAS** ซึ่งเป็นอัตราส่วน
         ที่เล็กกว่า 0.00001 ได้จริงโดยยังมีความหมายเต็ม ๆ
       · วัดจริง: เดือนก่อน ROAS = ฿0.01 / ฿2,000 = 0.000005 · เดือนนี้ = ฿0.02 / ฿2,000
         = 0.00001 → **ดีขึ้นเท่าตัว (+100%)** แต่ helper คืน null แดชบอร์ดจึงไม่บอกอะไรเลย
       · เป็นบทเรียนเดียวกับที่เคยพลาดมาแล้วสองรอบ: **ค่าที่เข้ามาไม่ได้มีหน่วยเดียวกัน
         ห้ามสมมติว่าเป็นเงิน** (รอบก่อนคือปัดผลต่างเป็นสตางค์แล้วทำ ROAS เพี้ยน)
       · ฝั่งจำนวนเงินไม่เสียอะไร — ยอดทุกตัวผ่าน `money_total()` หรือมาจาก `DECIMAL(12,2)`
         จึงเป็นพหุคูณของ 0.01 เสมอ "เล็กแต่ไม่ใช่ศูนย์" ที่น้อยกว่าสตางค์จึงเกิดไม่ได้ */
    if ($current === null || $previous === null || $previous == 0.0) {
        return null;
    }

    /* ⚠️⚠️⚠️ **ปัดสองชั้น** — ชั้นแรกล้างเศษของทศนิยมฐานสอง ชั้นสองคือการปัดที่ตั้งใจ
       · ปัญหา: `-99.95 − (-100.00)` ในภาษา PHP ได้ `0.04999999999999716` ไม่ใช่ `0.05`
         → ปัดหนึ่งตำแหน่งได้ **0.0%** ทั้งที่คำตอบคือ **+0.1%** (รายงานว่า "ไม่เปลี่ยนแปลง"
         ทั้งที่โตขึ้น) · เกิดเฉพาะตอนผลลัพธ์ตกตรงครึ่งหน่วยพอดี ซึ่งเป็นขอบที่การปัดต้องตัดสิน
       · ⚠️⚠️ **ห้ามแก้ด้วยการปัดผลต่างเป็นสตางค์** — เคยทำแล้วพังหนักกว่าเดิม:
         ฟังก์ชันนี้ถูกเรียกด้วย **ROAS** ซึ่งเป็นอัตราส่วน ไม่ใช่จำนวนเงิน
         ROAS 1.000 → 1.006 คือ **+0.6%** แต่การปัดผลต่างเป็นสตางค์ทำให้ `0.006` → `0.01`
         แล้วรายงาน **+1.0%** (วัดจริง) · ตัวเลขที่เข้ามาไม่ได้มีหน่วยเดียวเสมอไป
       · ปัดที่ **ผลลัพธ์** ด้วยความละเอียดสูงก่อน (10 ตำแหน่ง) จึงไม่ต้องรู้หน่วยของค่า
         และล้างเศษฐานสองได้ทุกกรณี */
    $percent = (($current - $previous) / abs($previous)) * 100;

    /* ⚠️ ชั้นแรกต้องละเอียดพอที่จะ **ไม่เปลี่ยนค่าจริง** — 10 ตำแหน่งยังหยาบไป:
       ยอด 20,000,000.02 → 20,010,000.02 คือ 0.049999999950% ซึ่งควรแสดง 0.0%
       แต่ถูกปัดชั้นแรกเป็น 0.05 แล้วกลายเป็น 0.1% · 12 ตำแหน่งยังล้างเศษฐานสองได้ครบ
       (ซึ่งอยู่ราว 1e-14) และไม่แตะค่าจริงในช่วงที่ยอดของระบบนี้เป็นไปได้ */
    return round(round($percent, 12), 1);
}
...
/**
 * ⭐⭐ ยอดรวมที่บวกในภาษา PHP ต้องปัดเป็น "สตางค์" ก่อนเอาไปใช้ต่อ
 *
 * เงินทุกช่องในระบบเป็น `DECIMAL(12,2)` (ละเอียดถึงสตางค์เท่านั้น) แต่ PHP เก็บเป็น
 * ทศนิยมฐานสอง การบวกทีละวันจึงเพี้ยนได้ในหลักที่ 10 กว่า ๆ ขณะที่ `SUM()` ของ
 * MySQL คำนวณแบบทศนิยมจริงและได้ค่าเป๊ะเสมอ — **สองหน้าที่ใช้คนละวิธีจึงตอบไม่ตรงกัน**
 *
 * ⚠️ วัดจริง (ส.ค. 2569 กรอก 7 วัน: 72328.46 · 63411.33 · 81239.59 · 60534.09 ·
 * 80981.37 · 77855.05 · 63650.11 — ตั้งเป้ารายได้ ฿500,000 ซึ่งลงตัวพอดีเป๊ะ):
 *   · แดชบอร์ดบวกเองได้ 499999.99999999994 → **"เป้ารายได้ 99.9%" · ไม่ขึ้น "🎉 ถึงเป้าแล้ว"**
 *     และบรรทัดล่างของการ์ดใบเดียวกันเขียนว่า "เหลือ 24 วัน · ต้องได้อีกวันละ ฿0"
 *   · หน้ารายปีกับไฟล์ Excel ใช้ `SUM()` ได้ 500000.00 → **"✓ ถึงเป้า (100.0%)"**
 *   ข้อมูลชุดเดียวกัน ร้านเดียวกัน วันเดียวกัน
 *
 * เกิดกับพฤติกรรมที่ตั้งใจทำ (เล็งเป้ากลม ๆ แล้วกรอกวันสุดท้ายให้ครบพอดี) ซึ่งเป็น
 * เส้นแบ่งที่สำคัญที่สุดของฟีเจอร์เป้าหมายพอดี · สุ่ม 200,000 เดือนพบว่าผลบวกของ PHP
 * ต่ำกว่าค่าเป๊ะ 32.1% ของเดือน (ทิศทางเดียว: ต่ำกว่าเท่านั้น)
 *
 * ⚠️ ปัด **ครั้งเดียวตอนจบ** ไม่ใช่ปัดทีละแถวระหว่างบวก — ปัดทีละแถวคือการเปลี่ยน
 * ตัวเลขของผู้ใช้ ส่วนตัวนี้แค่ลบเศษที่เกิดจากวิธีเก็บของภาษาเท่านั้น
 */
function money_total(float $value): float
{
 ...
 * ซึ่งไม่จริงในกรณีที่พบบ่อยพอกัน: **ทุกเดือนที่จบแล้วกำไรเท่ากันหมด**
 *
 ...
 * และตารางใต้การ์ดแสดง 7 แถวของเดือนที่จบแล้ว · การซ่อนการ์ดถูกแล้ว คำอธิบายผิด
 *
 * ⚠️ เดิมสตริงนี้ถูกคัดลอกไว้ 3 ที่ (`annual.php` · `overview.php` · ไฟล์ Excel)
 */
function extremes_not_comparable_text(): string
{
    return 'ยังเทียบไม่ได้ — ยังไม่มีเดือนที่ผลต่างกันให้เทียบ';
}
...
/**
 * ⭐⭐ ข้อความกำกับว่า "ไฟล์นี้มีข้อมูลถึงวันไหน"
 *
 * ⚠️⚠️ **ชีตรายวันตัดที่วันนี้ แต่หน้าประวัติไม่ตัด** — ทั้งสองอย่างตั้งใจ:
 *   · รายงานไม่ควรนับวันที่ยังไม่เกิดขึ้น (`daily_records` เป็นยอดจริงเท่านั้น)
 *   · หน้าประวัติต้องแสดงแถวเก่าที่ลงวันที่ล่วงหน้าไว้ ไม่งั้นลบมันไม่ได้อีกเลย
 *
 * ผลคือร้านที่มีแถวเก่าแบบนั้น จะเห็นจำนวนแถวบนจอกับในไฟล์ไม่เท่ากัน
 * **[เจ้าของระบบตัดสิน 2026-08-12] คงพฤติกรรมไว้ แต่ให้ไฟล์บอกวันตัดของตัวเอง**
 * — คนเปิดไฟล์จะได้รู้ว่าทำไมถึงไม่มีวันหลังจากนั้น โดยไม่ต้องเดา
 *
 * ⚠️ คืน null เมื่อไม่ได้ตัดอะไรเลย (ปีที่จบไปแล้ว) — เขียนกำกับทุกปีคือเสียงรบกวน
 * ที่ไม่บอกอะไรใหม่ หลักเดียวกับ `comparison_length_note()`
 */
function export_coverage_note(?string $coveredThrough, bool $wasTrimmed): ?string
{
    if (!$wasTrimmed || $coveredThrough === null || $coveredThrough === '') {
        return null;
    }

    $date = DateTimeImmutable::createFromFormat('!Y-m-d', $coveredThrough);
    if (!$date) {
        return null;
    }

    return 'ข้อมูลถึงวันที่ ' . formatThaiDate($coveredThrough)
        . ' — วันหลังจากนี้ยังไม่เกิดขึ้น จึงไม่นับรวมในรายงาน';
}
...
function normalize_money_string(string $raw): ?string
{
    ...
    //
    // ⚠️⚠️ ต้องระบุ U+FEFF เพิ่มเอง — `\s` ของ **JS ครอบให้** แต่ `\s` ของ PHP ไม่ครอบ
    // (ยูนิโค้ดไม่ได้จัด U+FEFF เป็น White_Space) · ไฟล์ที่ผ่าน Excel/Google Sheets
    // ติดอักขระตัวนี้มาได้ · ปล่อยไว้ = ค่าเดียวกัน "วางลงตารางแล้วอ่านออก แต่นำเข้า
    // เป็นไฟล์แล้วอ่านไม่ออก" ซึ่งอธิบายให้ผู้ใช้ไม่ได้ (บทเรียนเดียวกับตัวอ่านวันที่
    // ที่ตัดอักขระความกว้างศูนย์ทิ้งด้วยเหตุผลนี้ และกับ `trim_unicode_whitespace()`
    // ซึ่งตัด U+FEFF ให้อยู่แล้วทุกช่องที่ผู้ใช้พิมพ์)
    //
    // ⚠️ ห้ามเติม U+200B–U+200D — ฝั่ง JS ไม่ตัดให้ การเติมข้างเดียวคือสร้างรอยแยกใหม่
    $value = preg_replace('/[฿\s\x{00A0}\x{FEFF}]+/u', '', trim($value));
    if (!is_string($value)) {
        return null;
    }
...
/**
 * ⚠️⚠️ ต้องใช้ `trim_unicode_whitespace()` ไม่ใช่ `trim()` — เหตุผลเดียวกับชื่อร้าน
 *
 * ก๊อปอีเมลจาก LINE/แชท/Word มาวางมักติดช่องว่างที่มองไม่เห็นมาด้วย (NBSP, zero-width,
 * ช่องว่างญี่ปุ่น) ซึ่ง `trim()` ของ PHP ไม่ตัด · `is_valid_email()` จึงตอบว่ารูปแบบผิด
 * ทั้งที่บนจอดูถูกทุกตัวอักษร:
 *   · สมัครสมาชิก / ลืมรหัสผ่าน / เปลี่ยนอีเมล → "กรุณากรอกอีเมลที่ถูกต้อง"
 *   · เข้าสู่ระบบ (ช่องว่างอยู่ข้างหน้า) → "อีเมลหรือรหัสผ่านไม่ถูกต้อง" ทั้งที่ทั้งคู่ถูก
 *     แล้วการลองซ้ำ ๆ ไปชนตัวจำกัดจำนวนครั้ง
 *
 * ⚠️ เดิมเข้าระบบได้เมื่อช่องว่างอยู่ **ท้ายสุด** — เป็นความบังเอิญของ collation ของ MySQL
 * ที่มองข้ามตัวท้ายให้ ไม่ใช่การออกแบบ (ฝั่ง PHP ยังถือว่ารูปแบบผิดอยู่ดี ทางอื่นจึงล้มหมด)
 */
function normalize_email(string $email): string
{
    return strtolower(trim_unicode_whitespace($email));
}
