...
function change_percent(?float $current, ?float $previous): ?float
{
    ...
    /* ⚠️⚠️ ปัดผลต่างเป็นสตางค์ก่อนหาร — ค่าที่เอามาเทียบเป็นจำนวนเงินเสมอ
       · วัดจริง: `-99.95 − (-100.00)` ในภาษา PHP ได้ `0.04999999999999716`
         ไม่ใช่ `0.05` → หารแล้วปัดหนึ่งตำแหน่งได้ **0.0%** ทั้งที่คำตอบคือ **+0.1%**
       · อาการคือรายงานว่า "ไม่เปลี่ยนแปลง" ทั้งที่โตขึ้นจริง — เกิดเฉพาะตอนผลต่าง
         ตกตรงครึ่งหน่วยพอดี (0.05% · 0.15% · …) ซึ่งเป็นขอบที่การปัดต้องตัดสิน
       · ปัดที่ผลต่าง ไม่ใช่ปัดที่ผลลัพธ์ — ค่าที่หารแล้วไม่ใช่จำนวนเงินอีกต่อไป */
    return round((money_total($current - $previous) / abs($previous)) * 100, 1);
}
