...
final class EmptyShopWorkbookTest extends ControllerTestCase
{
    ...
    /**
     * ⭐⭐⭐ **ตัวกวาดทั้งไฟล์** — ร้านที่ไม่เคยกรอก ต้องไม่มีจำนวนเงินโผล่แม้แต่ช่องเดียว
     *
     * ⚠️⚠️ **นี่คือเทสต์ที่ควรมีตั้งแต่แรก** — เทสต์ตัวบนไล่ตรวจ "ชีตที่นึกออก" ทีละชีต
     * ซึ่งคือวิธีเดียวกับที่ทำให้พลาดมาแล้วสองรอบ: รอบแรกแก้ชีตรายเดือนชีตเดียวแล้วประกาศว่า
     * "ครบทุกจุดที่พิมพ์ตัวเลข" ทั้งที่เหลืออีก 3 ชีต · **ปัญหาไม่ได้อยู่ที่แก้ไม่ครบ
     * แต่อยู่ที่ไม่มีอะไรบอกได้ว่าครบหรือยัง**
     *
     * ตัวนี้ไม่ไล่ตามรายชื่อ — มันกวาด **ทุกชีต ทุกช่อง** ในไฟล์ที่ดาวน์โหลดจริง
     * แล้วถามคำถามเดียว: ช่องนี้เป็นช่องจำนวนเงิน/เปอร์เซ็นต์ไหม ถ้าใช่ ต้องว่าง
     * · เพิ่มชีตใหม่ในอนาคต **ถูกตรวจเองทันทีโดยไม่ต้องมาแก้เทสต์**
     *
     * ⚠️ รู้ได้ยังไงว่าช่องไหนคือ "จำนวนเงิน": ดูจาก **รูปแบบตัวเลขของเซลล์** ที่โปรแกรมตั้งไว้
     * (`"฿"#,##0.00` · `0.0"%"`) ไม่ใช่เดาจากชื่อคอลัมน์ · ช่องนับจำนวน (วันที่กรอก 0)
     * ใช้รูปแบบทั่วไป จึงไม่ถูกจับ ซึ่งถูกต้องตามกติกาที่ตกลงไว้
     *
     * ⚠️⚠️ ต้องกวาด **ข้อความ** ด้วย — จำนวนเงินที่อยู่ในประโยค (เช่นแถบประมาณการ
     * หรือบรรทัดสรุป) ไม่มีรูปแบบตัวเลขให้ดู แต่มันคือเลขที่ผู้ใช้อ่านเหมือนกัน
     */
    public function testNoMoneyCellAnywhereInTheWorkbookForAShopThatNeverRecorded(): void
    {
        $userId = $this->createUser('sweep@example.com', 'SweepPass123');
        $emptyShop = $this->createShop($userId, 'ร้านที่ไม่เคยกรอก');
        /* ⚠️ ต้องให้ **ทุกร้านของผู้ใช้** ว่างเปล่า ไม่ใช่แค่ร้านที่เลือกอยู่ —
           ชีต "เทียบร้าน" แสดงทุกร้านในบัญชี ถ้ามีร้านอื่นที่กรอกแล้ว ตัวกวาดจะไปเจอ
           ตัวเลขของร้านนั้นแล้วรายงานผิด (เจอมาแล้วตอนเขียนรอบแรก)
           · กรณี "ร้านหนึ่งมีข้อมูล อีกร้านไม่มี" ถูกคุมโดยเทสต์ตัวบนซึ่งเจาะดูเฉพาะ
             แถวของร้านที่ไม่เคยกรอก */
        $this->createShop($userId, 'ร้านที่ไม่เคยกรอกเหมือนกัน');

        $sheets = $this->workbookFor($this->startSession($userId, $emptyShop));
        $this->assertGreaterThan(1, count($sheets), 'ไฟล์มีชีตเดียว — น่าจะสร้างไม่สำเร็จ');

        $moneyFormats = ['"฿"#,##0.00', '0.0"%"'];
        $offenders = [];
        $scannedCells = 0;
        ...
        foreach ($sheets as $name => $sheet) {
            foreach ($sheet->getRowIterator() as $row) {
                foreach ($row->getCellIterator() as $cell) {
                    ...
                    /* (1) ช่องที่ถูกจัดรูปแบบเป็นจำนวนเงิน/เปอร์เซ็นต์ ต้องไม่มีตัวเลข
                       ⚠️ ถามรูปแบบ **หลัง** เช็กว่าเป็นตัวเลขแล้วเท่านั้น — การถามรูปแบบของเซลล์
                          แพงมาก ถามทุกช่องทำให้เทสต์ตัวเดียวใช้เวลาเป็นนาที */
                    if (is_numeric($value)) {
                        $format = $sheet->getStyle($coordinate)->getNumberFormat()->getFormatCode();
                        if (in_array($format, $moneyFormats, true)) {
                            // ⚠️ ใส่ป้ายของแถว (ช่อง A) มาด้วย — ไม่งั้นอ่านผลแล้วไม่รู้ว่าแถวไหน
                            $offenders[] = sprintf(
                                '%s!%s = %s   [แถว: %s]',
                                $name,
                                $coordinate,
                                (string)$value,
                                (string)$sheet->getCell('A' . $cell->getRow())->getValue()
                            );
                        }
                    ...
                    /* (2) จำนวนเงินที่ฝังอยู่ในประโยค — ยอมให้มีสัญลักษณ์ ฿ ได้ถ้าตามด้วย
                       ขีด (เช่น "กำไรสะสม –") แต่ห้ามตามด้วยตัวเลข */
                    if (is_string($value) && preg_match('/฿\s*[0-9]/u', $value) === 1) {
                        $offenders[] = sprintf('%s!%s = "%s"', $name, $coordinate, $value);
                    }
                }
            }
        }

        $this->assertGreaterThan(100, $scannedCells, 'กวาดได้ไม่กี่ช่อง — ตัวกวาดน่าจะไม่ได้เดินจริง');

        $this->assertSame(
            [],
            $offenders,
            "ร้านที่ยังไม่เคยกรอกข้อมูล แต่ไฟล์ยังพิมพ์จำนวนเงินออกมา:\n  "
            . implode("\n  ", $offenders)
            . "\n\n(ช่องพวกนี้ต้องเว้นว่าง — ตัวเลข 0 อ่านว่า \"ทำได้เท่านี้\" ไม่ใช่ \"ยังไม่มีข้อมูล\")"
        );
    }
}
