...
final class SharedHelperContractTest extends TestCase
{
    ...
    public static function changePercentProvider(): array
    {
        return [
            ...
            /* ⚠️⚠️ **ฐานที่เล็กแต่ไม่ใช่ศูนย์ ต้องยังเทียบได้** — เดิมแถวนี้คาดหวัง `null`
               ซึ่งล็อกเกณฑ์ `abs(prev) < 0.00001` ที่ **ผูกกับหน่วยเงิน** เอาไว้
               · ฟังก์ชันนี้ถูกเรียกด้วย ROAS ซึ่งเป็นอัตราส่วนที่เล็กกว่านั้นได้จริง
                 โดยยังมีความหมายเต็ม ๆ (ดูแถว "ROAS เล็กแต่เป็นบวก" ข้างล่าง)
               · ฝั่งเงินไม่เสียอะไร เพราะยอดทุกตัวเป็นพหุคูณของสตางค์เสมอ
                 "เล็กกว่าสตางค์แต่ไม่ใช่ศูนย์" จึงเกิดขึ้นไม่ได้ */
            'ฐานเป็นศูนย์เป๊ะ (เทียบไม่ได้จริง)' => [5000.0, 0.0, null],
            'ROAS เล็กแต่เป็นบวก — ดีขึ้นเท่าตัว' => [0.00001, 0.000005, 100.0],
            'ROAS เล็กมากและแย่ลงครึ่งหนึ่ง' => [0.000005, 0.00001, -50.0],
