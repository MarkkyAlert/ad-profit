...
final class DashboardServiceComparisonTest extends TestCase
{
    ...
    /**
     * ⭐⭐⭐ ROAS ที่เล็กมากแต่ **เป็นบวก** ต้องยังเป็นฐานเปรียบเทียบได้
     *
     * ⚠️⚠️ `change_percent()` เคยตัดฐานที่ `abs(prev) < 0.00001` ทิ้งเป็น "เทียบไม่ได้"
     * ซึ่งเป็นเกณฑ์ที่คิดมาจาก **จำนวนเงิน** — แต่ ROAS เป็น **อัตราส่วน** ที่เล็กกว่านั้น
     * ได้จริงโดยยังมีความหมายเต็ม ๆ
     *
     * วัดจริง: เดือนก่อน ฿0.01 / ฿2,000 = ROAS 0.000005 · เดือนนี้ ฿0.02 / ฿2,000 = 0.00001
     * → **ดีขึ้นเท่าตัว (+100%)** แต่แดชบอร์ดไม่แสดงอะไรเลย
     *
     * ⚠️ เป็นบทเรียนเดียวกับที่พลาดมาแล้วสองรอบ: ค่าที่ส่งเข้า `change_percent()`
     * ไม่ได้มีหน่วยเดียวกัน — ห้ามสมมติว่าเป็นเงิน
     */
    public function testATinyButPositiveRoasIsStillAComparisonBase(): void
    {
        $change = $this->changeOf([
            ['2026-07', 0.01, 2000.0],   // ROAS 0.000005
            ['2026-08', 0.02, 2000.0],   // ROAS 0.00001  → ดีขึ้นเท่าตัว
        ]);

        $this->assertArrayHasKey('roas', $change);
        $this->assertNotNull(
            $change['roas'],
            'ROAS ที่เล็กแต่เป็นบวกถูกตัดทิ้งเป็น "ไม่มีฐานเทียบ" ทั้งที่มีฐานและเปลี่ยนจริง'
        );
        $this->assertSame(100.0, $change['roas'], 'ROAS ดีขึ้นเท่าตัวต้องเป็น +100.0%');
    }

    /**
     * ⭐⭐ ทางตรงข้าม: ไม่ได้ยิงแอดเลยเดือนก่อน → ROAS ไม่มีอยู่จริง → ต้องเป็น null
     *
     * ⚠️ ถ้าขาดเทสต์ตัวนี้ การ "แก้" ที่ถอดด่านออกทั้งหมดจะผ่านหน้าตาเฉย
     * แล้วเดือนที่ไม่มี ROAS จะถูกเทียบด้วยศูนย์ ได้เปอร์เซ็นต์มหาศาลที่ไม่มีความหมาย
     */
    public function testAMonthWithoutAdSpendHasNoRoasToCompareWith(): void
    {
        $change = $this->changeOf([
            ['2026-07', 5000.0, 0.0],     // ไม่ได้ยิงแอด → ROAS ไม่มีค่า
            ['2026-08', 5000.0, 1000.0],
        ]);

        $this->assertArrayHasKey('roas', $change);
        $this->assertNull($change['roas'], 'เดือนก่อนไม่มี ROAS จึงเทียบเป็น % ไม่ได้');
    }
}
