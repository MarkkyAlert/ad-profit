...
- [ ] PHP **8.2+** (ไม่ใช่ 8.1 — `composer.json` บังคับ `>=8.2`)
  - Hostinger ที่ใช้อยู่คือ 8.3 ซึ่งผ่าน
- [ ] DB เป็น MySQL/MariaDB และรองรับ:
  - [ ] InnoDB + Foreign Key
  - [ ] `utf8mb4`
  - [ ] **collation `utf8mb4_unicode_520_ci`** (มีทั้ง MySQL 8 และ MariaDB 10.4 — ทดสอบแล้ว)
- [ ] PHP extensions ตามโค้ด:
  ...
  - [ ] ⚠️ **`zip`, `gd`, `dom`, `fileinfo`, `iconv`, `simplexml`, `xmlreader`, `xmlwriter`, `zlib`**
        — ตัวทำไฟล์ Excel (`phpoffice/phpspreadsheet`) ประกาศไว้เป็นข้อบังคับ
        **ขาดตัวใดตัวหนึ่ง `composer install` ล้มตั้งแต่แรก** ต้องเปิดใน hPanel ก่อน

### 2.1.1 ⚠️ ติดตั้งไลบรารีด้วย `--no-dev` เท่านั้น

```bash
composer install --no-dev --optimize-autoloader
```

**ห้ามใช้ `composer install` เปล่า ๆ บนเซิร์ฟเวอร์** — ไลบรารีสำหรับทดสอบ (PHPUnit)
ต้องการ PHP 8.4 ขึ้นไป บนโฮสต์ PHP 8.3 คำสั่งจะล้มทั้งคำสั่งและ **ไม่ได้ `vendor/` เลยสักตัว**

⚠️ อาการหลังจากนั้นอ่านยากมาก เพราะ **เว็บยังเปิดได้ปกติเหมือนไม่มีอะไรผิด**
แต่ **ดาวน์โหลดไฟล์ Excel พัง** และ **อีเมลรีเซ็ตรหัสผ่านส่งไม่ออก** (เงียบทั้งคู่)

### 2.2 ตรวจสิทธิ์ไฟล์/โฟลเดอร์ (Permission)
- [ ] ⚠️ **ตั้ง `LOG_FILE` เป็น path เต็มเสมอ อย่าปล่อยว่าง**
  - ปล่อยว่าง = ใช้โฟลเดอร์ temp ของระบบ ซึ่ง **เว็บกับ cron มักได้คนละโฟลเดอร์**
    (`TMPDIR` ต่างกัน) ผลคือ cron ไปกวาดโฟลเดอร์หนึ่ง แต่ log จริงโตอยู่อีกโฟลเดอร์
    ไม่มีใครลบตลอดไป
- [ ] PHP ต้อง “เขียนไฟล์ log ได้” ตาม path ที่กำหนดใน `LOG_FILE`
  - โค้ดจะสร้างโฟลเดอร์ของ `LOG_FILE` อัตโนมัติ (`includes/bootstrap.php`)
  - ⚠️ ถ้าเขียน **ตัวไฟล์** ไม่ได้ (เช่น root สร้างไว้ก่อน) ระบบจะถอยไปใช้ปลายทางปริยาย
    ของโฮสต์แทน — เว็บไม่พัง แต่ต้องรู้ว่า log ไปอยู่ที่ไหน
...
#### ⚠️⚠️ ลำดับสำคัญมาก: รัน migration **ก่อน** อัปโหลดโค้ดใหม่

ถ้าอัปโหลดโค้ดขึ้นไปก่อน ระบบจะปฏิเสธการบูตทันที (ตัวตรวจโครงสร้างเห็นว่ากติกา
เทียบชื่อร้านยังไม่เปลี่ยน) → **ทุกหน้ารวมทั้งหน้าเข้าสู่ระบบตอบ 503** และ cron
ทุกตัวจบด้วยข้อผิดพลาด ส่งอีเมลแจ้งเตือนหาเจ้าของทุกคืน

ข้อความที่จะเห็นใน log:
```
Column shops.name must use collation utf8mb4_unicode_520_ci
(found utf8mb4_unicode_ci) — run database/migrations
```

**ลำดับที่ถูก:** สำรอง DB → รัน migration → ตรวจว่า collation เปลี่ยนแล้ว → อัปโหลดโค้ด

#### ถ้า migration ล้มกลางคัน

ไฟล์นี้ทำ 2 ขั้น: (1) เปลี่ยนชื่อร้านที่จะชนกัน (2) เปลี่ยนกติกาเทียบชื่อ
ถ้าขั้นที่ 2 ล้ม (เช่นเนื้อที่ดิสก์ไม่พอ) DB จะค้างครึ่งทาง — ชื่อร้านเปลี่ยนแล้ว
แต่กติกายังเก่า และแอปจะไม่ยอมบูต

**วิธีกู้: รันไฟล์เดิมซ้ำอีกครั้ง** — ทดสอบแล้วว่าปลอดภัย รันซ้ำได้ไม่มีอะไรเปลี่ยนเพิ่ม
ไม่ต้องกู้จากไฟล์สำรอง

### 2.5 ⚠️ ตรวจว่าไฟล์ลับโหลดผ่านเว็บไม่ได้ (ทำทันทีหลังอัปโหลด)

โฟลเดอร์โปรเจกต์บน Hostinger คือโฟลเดอร์ที่เว็บเสิร์ฟตรง ๆ ไฟล์ `.htaccess`
ในโปรเจกต์กันไว้ให้แล้ว **แต่ต้องยืนยันด้วยตาว่ามันทำงานจริงบนโฮสต์นั้น**
(ถ้าโฮสต์เป็น nginx จะไม่อ่าน `.htaccess` เลย ต้องไปตั้งที่ตัวเซิร์ฟเวอร์แทน)
...
ถ้าอันไหนดาวน์โหลดได้ **ให้เปลี่ยนรหัสฐานข้อมูลและรหัสอีเมลทันที** แล้วค่อยแก้การตั้งค่า
...
> หมายเหตุ: `RATE_LIMIT_MAX_ATTEMPTS` และ `RATE_LIMIT_WINDOW_SECONDS` **อ่านจาก `.env` แล้ว**
> (ข้อความเดิมที่ว่า "เป็นค่าคงที่ ไม่ได้อ่านจาก .env" ไม่จริงอีกต่อไป)
>
> ค่าที่เพิ่มเข้ามาใหม่และปรับได้จาก `.env` เช่นกัน:
> `LOGIN_ACCOUNT_MAX_ATTEMPTS` · `LOGIN_ACCOUNT_WINDOW_SECONDS` · `LOGIN_ACCOUNT_MAX_DELAY_MS`
> (ตัวจำกัดการเดารหัสผ่านแบบ "หน่วงเวลา" ที่นับรวมทุกเครื่อง — ดู `.env.example`)
