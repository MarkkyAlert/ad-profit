...
> **จะขึ้น Hostinger?** → [HOSTINGER.md](HOSTINGER.md) (ทีละขั้น ทดลองจริงแล้ว)
> **จะแก้โค้ดต่อ?** → [DEVELOP.md](DEVELOP.md) (เตรียมเครื่อง + รันเทสต์)
...
### B.1 PHP
- ✅ **PHP 8.3 ขึ้นไป** — บังคับใน `composer.json` (`"php": ">=8.3"`)
  - ⚠️⚠️ **เคยเขียนไว้ว่า 8.1+ ซึ่งไม่จริงแล้ว** · ตัวที่บังคับคือ
    `maennchen/zipstream-php` ที่ `phpoffice/phpspreadsheet` ลากมา ซึ่ง require
    `php-64bit ^8.3` → ต่ำกว่านี้ `composer install` ล้มตั้งแต่แรก = ไม่มี `vendor/`
    = เว็บเปิดไม่ขึ้นทั้งเว็บ
- ⚠️ **ถ้าจะรันชุดทดสอบด้วย ต้อง PHP 8.4.1 ขึ้นไป** (PHPUnit 13 บังคับ)
  - ⚠️ **PHP ที่มากับ XAMPP (8.2.4) ใช้ไม่ได้ทั้งสองอย่าง** ต้องติดตั้งแยก
  - การเตรียมเครื่องเพื่อแก้โค้ด → [DEVELOP.md](DEVELOP.md)

### B.2 Web Server
- ✅ Web Server ที่รัน PHP ได้ เช่น Apache (XAMPP) หรือ Shared Hosting ที่รองรับ PHP
- ⚠️⚠️⚠️ **ข้อความเดิมเขียนว่า "ไม่พบไฟล์ `.htaccess` ในโปรเจกต์" — ไม่จริงแล้ว และอันตราย**
  - ตอนนี้มี **`.htaccess` 9 ไฟล์** และตัวที่รากคือ **สิ่งเดียว** ที่กันไม่ให้
    `https://โดเมน/.env` ถูกดาวน์โหลด (รหัสฐานข้อมูล + รหัสอีเมล)
  - **ต้องเปิด `mod_rewrite`** — ใช้กันโฟลเดอร์ภายใน · กัน `.git/` · และพาไปหน้า 404 ของแอป
  - ⚠️ ตอนอัปโหลดต้อง **เปิด "แสดงไฟล์ซ่อน" ในโปรแกรม FTP** ไม่งั้นไฟล์พวกนี้ไม่ขึ้นไป
  - หลังขึ้นเซิร์ฟเวอร์ต้องรัน `php tools/check-live.php https://โดเมน` ยืนยันเสมอ
...
- `/database/sample_data.sql` — ข้อมูลตัวอย่าง ⚠️ **เฉพาะเครื่องตัวเอง** (มี `DELETE` ล้างข้อมูลเดิม + มีบัญชีทดสอบพร้อมรหัสผ่านฝังอยู่)
...
- [ ] Import ไฟล์ `database/sample_data.sql` — **เฉพาะเครื่องตัวเองเท่านั้น**

> ⚠️⚠️⚠️ **ห้ามนำเข้าไฟล์นี้บนเซิร์ฟเวอร์จริงเด็ดขาด**
> มีบัญชี `demo@example.com` / `team@example.com` **พร้อมรหัสผ่านฝังอยู่ในไฟล์**
> = ใครที่เคยเห็นซอร์สโค้ดก็เข้าระบบของคุณได้
> และไฟล์นี้ยัง `DELETE` ข้อมูลเดิมทิ้งก่อนใส่ข้อมูลตัวอย่างด้วย

> ⚠️ `database/sample_data.sql` มีคำสั่ง `DELETE FROM ...` เพื่อล้างข้อมูลเดิมของระบบชุดนี้ก่อนใส่ข้อมูลใหม่
>
> ⚠️⚠️ ไฟล์นี้เพิ่มผู้ใช้ตัวอย่าง `demo@example.com` และ `team@example.com`
> **พร้อมรหัสผ่านที่ฝังอยู่ในไฟล์** — สะดวกตอนลองเล่นบนเครื่องตัวเอง
> แต่ **บนเซิร์ฟเวอร์จริงคือช่องให้ใครก็ได้เข้าระบบ** ห้ามนำเข้าเด็ดขาด
>
