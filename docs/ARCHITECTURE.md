...
- `/database/`
  ...
  - `sample_data.sql` ข้อมูลตัวอย่าง ⚠️ **มีบัญชีทดสอบพร้อมรหัสผ่าน — ห้ามนำเข้าบนเซิร์ฟเวอร์จริง**
...
- บันทึก/แก้ไข/ลบรายการรายวัน: TX + `FOR UPDATE` lock แถวเพื่อกันข้อมูลชนกัน
- เปลี่ยนชื่อร้าน/ลบร้าน: TX + `FOR UPDATE` lock แถวเพื่อกันข้อมูลชนกัน
- เปลี่ยนอีเมล/รหัสผ่าน: TX + `FOR UPDATE` lock user row
- บันทึก/ลบเป้าหมาย: TX เพื่อกันข้อมูลชนกัน
