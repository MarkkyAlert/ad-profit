...
class RecordService
{
    ...
    /** ข้อความเดียวของ "ไฟล์ไม่ใช่ UTF-8" — บอกวิธีแก้ให้ตรง ๆ ไม่ใช่เดาสาเหตุ */
    public const IMPORT_ENCODING_ERROR = 'ไฟล์นี้ไม่ได้บันทึกเป็น UTF-8 ตัวอักษรไทยจึงอ่านไม่ออก '
        . 'กรุณาเปิดใน Excel แล้ว Save as "CSV UTF-8 (Comma delimited)" '
        . 'หรือใช้ไฟล์ที่ดาวน์โหลดจากหน้าประวัติ';
    ...
    public function parseImportCsv(string $content, ?int $maxRows = null): array
    {
        ...
        /* ⚠️⚠️ ไฟล์ที่ไม่ใช่ UTF-8 ต้องบอกตั้งแต่ต้น — ไม่ใช่ปล่อยไปตายที่ฐานข้อมูล
           · Excel บนวินโดวส์ภาษาไทยบันทึก CSV เป็น CP874 (ANSI) โดยปริยาย
           · ด่านหัวตารางด้านล่างจับได้เฉพาะตอน **หัวเป็นภาษาไทย** เพราะชื่อคอลัมน์
             จะจับคู่ไม่ติด · แต่หัวภาษาอังกฤษ (date/revenue/ad_cost/note) เป็น ASCII
             ล้วน จึงผ่านด่านนั้นไปได้ แล้วไบต์ไทยของ **คอลัมน์โน้ต** ไปตายที่ MySQL
           · วัดจริง: หัวอังกฤษ + โน้ตไทยแบบ CP874 → บันทึก 0 แถว พร้อมข้อความ
             "ไม่สามารถบันทึกข้อมูลได้" ซึ่งไม่บอกอะไรเลยว่าต้องทำอะไรต่อ
             (ไฟล์เดียวกันถ้าโน้ตเป็น UTF-8 บันทึกได้ปกติ — พิสูจน์ว่าเป็นเรื่องการเข้ารหัส)
           · ⚠️ ไฟล์จากระบบอื่น (แพลตฟอร์มขายของ/แอดแพลตฟอร์ม) มักมีหัวเป็นอังกฤษ
             ทางนี้จึงเป็นทางที่ผู้ใช้เดินจริงบ่อย ไม่ใช่กรณีหายาก
           ⚠️ ข้อความต้องบอกสาเหตุ **ที่รู้แน่** — ตรงนี้รู้ชัดว่าเป็นการเข้ารหัส
             ต่างจากด่านหัวตารางซึ่งเป็นได้ทั้ง encoding/ตัวคั่น/ไม่ใช่ CSV */
        if (!mb_check_encoding($content, 'UTF-8')) {
            return $fail(self::IMPORT_ENCODING_ERROR);
        }
