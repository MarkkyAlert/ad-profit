    ...
    public function buildMonthlyHeatmap(int $userId, int $shopId, int $year, ?string $today = null): array
    {
        ...
        foreach ($years as $gridYear) {
            ...
            for ($month = 1; $month <= 12; $month++) {
                ...
                //
                // ⚠️⚠️ เดือนปัจจุบันยังไม่จบ ยอดจึงเป็นแค่บางส่วน (ตัดที่วันนี้) เอาไปเทียบกับ
                // เดือนที่จบแล้วตรง ๆ ไม่ได้ · วัดจริง: ร้านที่ทำกำไรวันละ ฿1,000 เท่ากันทุกวัน
                // ช่อง ส.ค. (กรอกถึงวันที่ 7) ได้ความเข้มต่ำสุดที่ระบบมี ขณะที่เดือนอื่นเข้ม
                // ปานกลางทั้งหมด → กริดที่มีไว้ตอบว่า "เดือนไหนขายดี" กลับบอกว่า ส.ค. แย่สุด
                // ทั้งที่การ์ด "เดือนกำไรแย่สุด" เหนือมันบนจอเดียวกันตั้งใจไม่ตัดสินเดือนนี้
                // (หลักเดียวกับที่เจ้าของระบบตัดสินไว้เมื่อ 2026-08-05)
                $row[$month] = [
                    'month' => $month,
                    'profit' => $totals !== null
