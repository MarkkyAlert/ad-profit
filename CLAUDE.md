...
- ⚠️ **ตัวจำกัดต้อง "จองก่อนตรวจ" (reserve-then-act) ไม่ใช่ "ถามก่อนแล้วค่อยนับ"** — ระหว่างอ่านตัวนับกับเขียนตัวนับมี `password_verify()`/`password_hash()` (~100 มิลลิวินาที) คั่นอยู่ ยิงพร้อมกัน 40 ครั้งจึงผ่านเข้าไป 28 ครั้งทั้งที่เพดาน 5 (วัดจริง) · ตอนนี้ `login`/`register`/`password_reset`/`reset_password` `reserveAttempt()` ก่อนทั้งหมด
  ...
  · ⚠️ **bucket ต่อ (IP + อีเมล) ยิงไม่ออกในทางปฏิบัติ** — เพดานเท่ากัน (5/60 วิ) และ `login_ip` นับ *ทุก* อีเมลจาก IP นั้น ค่าจึง ≥ ตัวนับต่อบัญชีเสมอ ชนเพดานก่อนหรือพร้อมกันเสมอ → **การไล่เดาบัญชีเดียวจากหลาย IP ไม่มีเพดานของตัวเอง** (IP ใหม่เริ่มนับ 0) · การเพิ่ม bucket ที่ผูกกับอีเมลล้วนแลกมาด้วยการเปิดช่องให้คนร้ายจงใจล็อกบัญชีเหยื่อทิ้ง — **เป็นการตัดสินใจของเจ้าของระบบ อย่าแก้เงียบ ๆ**
