...
- ⚠️⚠️⚠️ **เขียนกติกาซ้ำ = จุดที่พังแน่นอน — ต้องเรียกกติกาตัวเดิมเสมอ**
  · ✅ **แก้ที่ต้นเหตุแล้ว: `month_is_unfinished()` ใน `includes/functions.php` คือจุดเดียวของกติกา "เดือนนี้จบหรือยัง"** (เดิมเขียนไว้ 3 ที่) · `SharedHelperContractTest` ล็อกกติกา 11 กรณีขอบ **และมีตัวกวาดห้ามเขียนซ้ำ** (`testNobodyReimplementsTheUnfinishedMonthRule`) ซึ่งแดงแม้โค้ดที่เขียนซ้ำจะให้ผลถูกต้องทุกประการ — เพราะปัญหาอยู่ที่วันที่มีคนแก้ที่หนึ่งแล้วอีกที่ไม่ตาม
