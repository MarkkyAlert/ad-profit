...
<section class="rounded-xl border border-slate-700 bg-slate-800 p-5">
    <div class="flex flex-wrap items-end justify-between gap-4">
        <div>
            ...
            <p class="mt-2 text-sm text-slate-300">ตารางรายการของเดือนที่เลือก พร้อมแก้ไข/ลบได้ทันที</p>
        </div>

        <div class="flex flex-wrap items-center gap-2">
            <form method="get" action="<?= e(app_url('/history.php')) ?>" class="flex flex-wrap items-center gap-2">
                <label for="month" class="text-sm text-slate-300">เลือกเดือน</label>
                <input
                    id="month"
                    name="month"
                    type="month"
    ...
    <div class="mt-5 overflow-x-auto">
        <table class="min-w-full text-sm">
            ...
            <tr class="border-t border-slate-600 bg-slate-900/60 font-semibold">
                ...
                <td class="px-3 py-2 text-violet-400"><?= e(formatRoas(isset($totals['avg_roas']) ? (is_null($totals['avg_roas']) ? null : (float)$totals['avg_roas']) : null)) ?></td>
